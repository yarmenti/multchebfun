...
class ChebyshevFun(object):
    ...
    def __call__(self, *x):
        ...
        if scaled_x.min() <= -1.:
            raise Exception("A value in x is lower that the lower bound of the interpolated function.")

        if scaled_x.max() >= 1.:
            raise Exception("A value in x is higher that the upper bound of the interpolated function.")
